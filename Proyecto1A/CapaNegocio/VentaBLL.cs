...
namespace Proyecto1A.CapaNegocio
{
    public class VentaBLL
    {
        ...
        public static RepuestaOperacion ValidarVenta(Venta venta, List<DetalleVenta> detalles)
        {
            // 1) Validar existencia del objeto Venta
            if (venta == null)
                return new RepuestaOperacion { Exito = false, Mensaje = "Venta no válida." };

            // 2) Validar cliente
            if (venta.Id_Cliente <= 0)
                return new RepuestaOperacion { Exito = false, Mensaje = "Debe seleccionar un cliente." };

            // 3) Validar tipo de pago
            if (venta.Id_TipoPago <= 0)
                return new RepuestaOperacion { Exito = false, Mensaje = "Debe seleccionar un tipo de pago." };

            // 4) Validar detalles
            if (detalles == null || detalles.Count == 0)
            {
                return new RepuestaOperacion
                {
                    Exito = false,
            ...
            if (venta.MontoTotal <= 0)
                return new RepuestaOperacion { Exito = false, Mensaje = "El total de la venta debe ser mayor a cero." };
